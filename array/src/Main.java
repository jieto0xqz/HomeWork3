...
public class Main {
    ...
    public static void raiseException(Integer exceptionId) throws Exception { // Подсказка: throws Exception в итоге надо поменять на перечисление имен исключений
        ...
        Map<Integer, Exception> exceptionMap = Map.of(
                1, new SocketException(),
                2, new ClassNotFoundException(),
                3, new AccessDeniedException(null),
                4, new ExportException(null)
        );
        throw exceptionMap.get(exceptionId);
    }
